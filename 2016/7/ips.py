...
inputs = '''dnwtsgywerfamfv[gwrhdujbiowtcirq]bjbhmuxdcasenlctwgh
...
hypernet_sequence = re.compile(r'(\[\S*?\])')


def supports_tls(ip_segments):
...
def has_aba(ip_segments):
    ...
    for segment in ip_segments:
        for i in range(len(segment) - 2):
            ...
            c = segment[i + 2]
            if a == c and a != b:
                abas.append('{}{}{}'.format(a, b, c))

    return abas


def has_bab(ip_segments, abas):
    for aba in abas:
...
# part 1
# count = 0
# for ip in inputs.splitlines():
#     outside = re.sub(hypernet_sequence, ' ', ip).split()
#     inside = [address[1:-1] for address in re.findall(hypernet_sequence, ip)]
#
#     if supports_tls(outside) and not supports_tls(inside):
#         count += 1
#
# print(count)
...
for ip in inputs.splitlines():
    ...
    abas = has_aba(outside)
    if abas and has_bab(inside, abas):
        count += 1
