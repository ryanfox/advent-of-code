...
inputs = '''bot 88 gives low to bot 51 and high to bot 42
...
gives = re.compile(r'bot (\d+) gives low to (bot|output) (\d+) and high to (bot|output) (\d+)')
assignment = re.compile(r'value (\d+) goes to bot (\d+)')
...
class Bot:
    ...
    def add(self, value):
        ...
        if len(self.inbox) == 2:
            ...
            if sorted(self.inbox) == [17, 61]:
                print(self.id)

            # need to move my chips now
            self.low_target.add(sorted(self.inbox)[0])
            self.high_target.add(sorted(self.inbox)[1])
            self.inbox = []
...
for instruction in inputs:
    if gives.match(instruction):
        bot, low_type, low_target, high_type, high_target = gives.match(instruction).groups()
        ...
        if low_target not in bots:
            bots[low_target] = Bot(low_target)

        if high_target not in bots:
            bots[high_target] = Bot(high_target)

        if low_type == 'bot':
            bots[bot].low_target = bots[low_target]
        else:
            if low_target not in outputs:
                outputs[low_target] = Output(low_target)

            bots[bot].low_target = outputs[low_target]

        if high_type == 'bot':
            bots[bot].high_target = bots[high_target]
        else:
            if high_target not in outputs:
                outputs[high_target] = Output(high_target)

            bots[bot].high_target = outputs[high_target]

for instruction in inputs:
    if assignment.match(instruction):
        value, bot = (int(number) for number in assignment.match(instruction).groups())
...
print(outputs[0].inbox[0] * outputs[1].inbox[0] * outputs[2].inbox[0])
