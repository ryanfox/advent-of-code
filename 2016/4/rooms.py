...
inputs = '''aczupnetwp-dnlgpyrpc-sfye-dstaatyr-561[patyc]
...
def is_real(room):
    letters = Counter(''.join(room[0]))
    most_common = letters.most_common()

    # need to sort alphabetically first, then by descending frequency
    # python sort guaranteed to be stable so tied letters won't be switched around in second sort
    counts = sorted(sorted(most_common, key=lambda x: x[0]), key=lambda y: y[1], reverse=True)[:5]
    return ''.join(count[0] for count in counts) == room[2]


def rot(word, distance):
    rotated = []
    ...
    for letter in word:
        ...
        new_letter = chr(((ord(letter) + (distance % 26)) % 97 % 26) + 97)
...
rooms = [room for room in inputs if is_real(room)]
print(sum(room[1] for room in rooms))


# part 2
for room in rooms:
    # printed the whole list, ctrl+F'd to north, found this garbled string.  Sector is right, though
    if rot(' '.join(room[0]), room[1]).lower() == 'northpole nobject nstorage':
        print(room)
